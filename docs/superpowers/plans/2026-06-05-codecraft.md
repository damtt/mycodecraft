...
export default function PlayersScreen() {
  ...
  const submit = () => {
    if (name.trim() === '') return; // guards Enter-key implicit submission
      ...
      <div className="flex flex-wrap justify-center gap-6">
        ...
        {creating ? (
          <Panel className="flex w-64 flex-col gap-3">
            {/* Real <form> so Enter submits — the natural move after typing a name.
                ⚠️ Every non-submit button inside MUST be type="button". */}
