...
export default function EditorPane({ initialValue, onChange, onFocusChange, className = '' }: EditorPaneProps) {
    ...
    <div className={`flex min-h-0 flex-col overflow-hidden rounded-lg bg-[#1e1e2e] p-1 ${className}`}>
      ...
      <div className="relative min-h-0 flex-1">
        <div className="absolute inset-0">
          <CodeEditor ref={editorRef} initialValue={initialValue} onChange={onChange} onFocusChange={onFocusChange} />
        </div>
      </div>
    </div>
  );
